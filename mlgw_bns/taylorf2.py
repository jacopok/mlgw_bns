...
def _make_compute_quadrupole_yy(xp):
    """Build :func:`compute_quadrupole_yy` over an array namespace ``xp``
    (``numpy`` or ``jax.numpy``). Shared source for the numba-jitted
    numpy path here and the JAX port in :mod:`mlgw_bns.jax_predict`.

    The ``lam <= 0`` branch is expressed as ``xp.where`` (rather than a
    Python ``if``) so the same code works under ``jax.vmap`` tracing; the
    ``xp.log`` argument is pre-clamped to avoid a spurious ``log`` of a
    non-positive number on the branch that is discarded anyway.
    """

    def compute_quadrupole_yy(lam):
        loglam = xp.log(xp.where(lam > 0.0, lam, 1.0))
        logCQ = (
            0.194
            + 0.0936 * loglam
            + 0.0474 * loglam ** 2
        ...
        return xp.where(lam <= 0.0, 1.0, xp.exp(logCQ))

    return compute_quadrupole_yy


_compute_quadrupole_yy_np = njit(cache=True)(_make_compute_quadrupole_yy(np))
...
def compute_quadrupole_yy(lam: float) -> float:
    ...
    return _compute_quadrupole_yy_np(lam)
...
def _make_phift7hpn_complete(xp):
    """Build the 7.5PN tidal phase correction (``https://arxiv.org/abs/2005.13367``)
    generically over an array namespace ``xp``, as a function of the PN
    velocity ``v`` directly (mass-independent; the caller converts from
    ``(f, M)`` to ``v``). Shared with :mod:`mlgw_bns.jax_predict`.
    """

    def phift7hpn_complete(v, eta, Lama, Lamb):
    ...
    return phift7hpn_complete


_phift7hpn_complete_np = njit(cache=True)(_make_phift7hpn_complete(np))
...
def PhifT7hPNComplete(
    ...
    """
    v = np.power(np.abs(np.pi * M * f * SUN_MASS_SECONDS), 1.0 / 3.0)
    return _phift7hpn_complete_np(v, eta, Lama, Lamb)


def _make_phifqm3hpn(xp, compute_quadrupole_yy):
    """Build the 3.5PN quadrupole-monopole self-spin phase term
    (Eq. 50-52 of `arXiv:1812.07923 <https://arxiv.org/abs/1812.07923>`_)
    generically over an array namespace ``xp``, as a function of the PN
    velocity ``v`` directly. Only the aligned (z) spin components enter
    this term, matching the original's unused ``s1x``/``s1y``/``s2x``/
    ``s2y`` arguments. ``compute_quadrupole_yy`` is injected so the
    caller controls whether it is the jitted-numpy or plain (JAX) core.
    Shared with :mod:`mlgw_bns.jax_predict`.
    """

    def phifqm3hpn(v, eta, s1z, s2z, Lam1, Lam2):
        v2 = v * v
        delta = xp.sqrt(1.0 - 4.0 * eta)
        X1 = 0.5 * (1.0 + delta)
        X2 = 0.5 * (1.0 - delta)
        ...
        PhifQM += -75.0 / (8.0 * eta) * a2CQ_p_a2CQ * v2 * xp.pi  # Tail
        return PhifQM

    return phifqm3hpn


_phifqm3hpn_np = njit(cache=True)(_make_phifqm3hpn(np, _compute_quadrupole_yy_np))
...
def PhifQM3hPN(
    ...
    """
    v = np.power(np.abs(np.pi * M * f * SUN_MASS_SECONDS), 1.0 / 3.0)
    return _phifqm3hpn_np(v, eta, s1z, s2z, Lam1, Lam2)


def _make_phif5hpn(xp, phif3hpn):
    """Build the 5.5PN point-mass + spin phase (`arXiv:1904.09558
    <https://arxiv.org/abs/1904.09558>`_) generically over an array
    namespace ``xp``, as a function of ``v`` directly. ``phif3hpn`` (the
    lower-order phase, see :func:`_make_phif3hpn`) is injected rather than
    recomputed, so numpy gets the jitted core and JAX gets the plain one.
    Shared with :mod:`mlgw_bns.jax_predict`.

    The ``c_21_3PN``/``c_22_4PN``/``c_22_5PN``/``a_6_c`` coefficients of
    the original implementation are identically zero (unfilled
    higher-order placeholders); the terms they multiply are dropped here
    rather than carried as literal ``+ 0.0 * (...)``, which does not
    change the floating-point result.
    """

    def phif5hpn(v, eta, s1x, s1y, s1z, s2x, s2y, s2z, Lam, dLam):
        v2 = v * v
        v4 = v2 * v2
        v5 = v4 * v
        v6 = v2 * v4
        ...
        phi_35pn = phif3hpn(v, eta, s1x, s1y, s1z, s2x, s2y, s2z, Lam, dLam)

        coef_8pn = (
            -36946947827.5 / 1601901100.8 * eta * eta * eta * eta
            + 51004148102.5 / 1310646355.2 * eta3
            + (30060067316599.7 / 57668439628.8 - 39954.5 / 2721.6 * xp.pi * xp.pi)
            * eta
            * eta
            + (
                -567987228950352.7 / 128152088064.0
                - 532292.8 / 396.9 * EULER_GAMMA
                + 930221.5 / 5443.2 * xp.pi * xp.pi
                - 142068.8 / 44.1 * log2
                + 2632.5 / 4.9 * log3
            )
            * eta
            - 9049.0 / 56.7 * xp.pi * xp.pi
        ...
        coef_9pn = xp.pi * (
            ...
            + (2255.0 / 6.0 * xp.pi * xp.pi - 149291726073.5 / 13412044.8) * eta
            - 640.0 / 3.0 * xp.pi * xp.pi
            - 1369.6 / 2.1 * EULER_GAMMA
            + 10534427947316.3 / 1877686272.0
            - 2739.2 / 2.1 * log2
        )

        coef_log9pn = -3 * 1369.6 / 6.3 * xp.pi
        ...
        coef_10pn = (
            ...
            * (
                ...
                * eta
                * eta
                - (1272143474037195162.1 / 67631771583129.6)
                * eta
                * eta
                * eta
                * eta
                ...
                * eta
                + (
                    ...
                    + 41579551.7 / 90316.8 * xp.pi * xp.pi
                ...
                * eta
                + (
                    ...
                    - 545142954.7 / 812851.2 * xp.pi * xp.pi
                    + 15994339707.7 / 1833678.0 * log2
                    + (892417.5 / 313.6) * log3
                )
                * eta
                + (57822311.5 / 304819.2) * xp.pi * xp.pi
        ...
        coef_11pn = xp.pi * (
            ...
            + (
                -1064790.5 / 3628.8 * eta * eta
                + 4501578.5 / 14515.2 * eta
                - 9439.0 / 56.7
            )
            * xp.pi
            * xp.pi
        ...
        coef_log11pn = -3 * xp.pi * (134666.2 / 170.1 * eta + 355801.1 / 2381.4)
...
def _make_phif3hpn(xp):
    """Build the 3.5PN point-mass + spin + tidal phase generically over an
    array namespace ``xp``, as a function of ``v`` directly. Shared with
    :mod:`mlgw_bns.jax_predict` (adapted from
    https://bitbucket.org/dailiang8/gwbinning/src/master/).

    The tidal term is computed unconditionally rather than guarded by
    ``if Lam != 0.0 or dLam != 0.0`` as in the original: with
    ``Lam == dLam == 0`` the formula evaluates to exactly zero anyway
    (same as the guarded branch), and an unconditional expression is
    required for this to work under ``jax.vmap`` tracing.
    """

    def phif3hpn(v, eta, s1x, s1y, s1z, s2x, s2y, s2z, Lam, dLam):
        vlso = 1.0 / xp.sqrt(6.0)
        delta = xp.sqrt(1.0 - 4.0 * eta)
        v2 = v * v
        v3 = v2 * v
        v4 = v2 * v2
        ...
        phis_3PN = xp.pi * (3760.0 * SL + 1490.0 * dSigmaL) / 3.0 + pn_ss3
        phis_35PN = (
        ...
        pointmass = (
            ...
            * v4
            + (38645.0 / 756.0 * xp.pi - 65.0 / 9.0 * eta * xp.pi - ga)
            * (1.0 + 3.0 * xp.log(v / vlso))
            * v5
            + (
                11583231236531.0 / 4694215680.0
                - 640.0 / 3.0 * xp.pi ** 2
                - 6848.0 / 21.0 * (EULER_GAMMA + xp.log(4.0 * v))
                + (-15737765635.0 / 3048192.0 + 2255.0 * xp.pi ** 2 / 12.0) * eta
                + 76055.0 / 1728.0 * eta2
...
_phif3hpn_np = njit(cache=True)(_make_phif3hpn(np))
_phif5hpn_np = njit(cache=True)(_make_phif5hpn(np, _phif3hpn_np))
...
def Phif3hPN(
    ...
    return _phif3hpn_np(v, eta, s1x, s1y, s1z, s2x, s2y, s2z, Lam, dLam)
...
def Phif5hPN(
    ...
    return _phif5hpn_np(v, eta, s1x, s1y, s1z, s2x, s2y, s2z, Lam, dLam)
...
def _make_taylorf2_psi(xp):
    """Build :func:`phase_5h_post_newtonian_tidal` in **natural units**
    (``v = (pi f_natural)^{1/3}``, mass-independent) and restricted to
    aligned spins, generically over an array namespace ``xp``.
    ...
    phif5hpn = _make_phif5hpn(xp, phif3hpn)
    phift7hpn_complete = _make_phift7hpn_complete(xp)
    phifqm3hpn = _make_phifqm3hpn(xp, compute_quadrupole_yy)

    def taylorf2_psi(f_natural, eta, chi1, chi2, lam1, lam2):
        v = xp.abs(math.pi * f_natural) ** (1.0 / 3.0)
        phi5 = phif5hpn(v, eta, 0.0, 0.0, chi1, 0.0, 0.0, chi2, 0.0, 0.0)
        phit = phift7hpn_complete(v, eta, lam1, lam2)
        phiqm = phifqm3hpn(v, eta, chi1, chi2, lam1, lam2)
