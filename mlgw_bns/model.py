    ...
    **model_kwargs
    ...
    def _mode_amplitudes_and_phases(
        ...
        source : str
            ...
            * ``"eob"``: a single batched call into TEOBResumS.
        ...
        if source == "eob":
            ...
            # One TEOBResumS call for every mode at once. Beyond being four
            # times cheaper, this is the convention the training data was
            # generated with: `all_modes_amplitude_phase` lowers the ODE
            # start frequency by the highest-m mode of the whole set, so
            # asking mode by mode would integrate each from a different
            # starting point and shift the phases at the ~1e-3 level.
            eob_waveforms = self.mode_models[
                self.modes[0]
            ].waveform_generator.all_modes_amplitude_phase(
                params_teob, list(self.modes), f_natural
            )
        ...
        for idx, mode in enumerate(self.modes):
            if source == "post_newtonian":
                ...
                _f_spa, amp, phase = eob_waveforms[mode]
            else:
                ...
                phase = phase + 2 * np.pi * (frequencies - reference_frequency_hz) * ts_scaled
